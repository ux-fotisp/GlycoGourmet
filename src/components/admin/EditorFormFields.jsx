import React from 'react';
import Button from '../ui/Button';
import Input from '../ui/Input';
import TagChip from '../ui/TagChip';
import IngredientSelector from './IngredientSelector';
import ImageUploader from './ImageUploader';

/**
 * Renders form fields to capture recipe titles, instructions, tags, and serving targets.
 */
export const EditorFormFields = ({
  formData,
  setFormData,
  onPublish,
  onSaveDraft
}) => {
  const handleBasicsChange = (field, val) => {
    setFormData(prev => ({
      ...prev,
      [field]: val
    }));
  };

  const handleTagToggle = (tag) => {
    setFormData(prev => {
      const tags = prev.tags.includes(tag)
        ? prev.tags.filter(t => t !== tag)
        : [...prev.tags, tag];
      return { ...prev, tags };
    });
  };

  const handleIngredientsChange = (newIngredientsList) => {
    setFormData(prev => ({
      ...prev,
      ingredients: newIngredientsList
    }));
  };

  const handleAddIngredientPlaceholder = () => {
    setFormData(prev => {
      const list = [...prev.ingredients];
      list.push({ ingredientId: 'atlantic-salmon', amount: 1, unit: 'oz', prepState: 'raw' });
      return { ...prev, ingredients: list };
    });
  };

  const handleRemoveIngredient = (index) => {
    setFormData(prev => {
      const list = prev.ingredients.filter((_, i) => i !== index);
      return { ...prev, ingredients: list };
    });
  };

  const handleAddStep = () => {
    setFormData(prev => ({
      ...prev,
      steps: [...prev.steps, { title: '', description: '', timer: null }]
    }));
  };

  const handleRemoveStep = (index) => {
    setFormData(prev => ({
      ...prev,
      steps: prev.steps.filter((_, i) => i !== index)
    }));
  };

  const handleStepChange = (index, field, value) => {
    setFormData(prev => {
      const list = [...prev.steps];
      list[index] = {
        ...list[index],
        [field]: field === 'timer' ? (value ? parseInt(value) || null : null) : value
      };
      return { ...prev, steps: list };
    });
  };

  const availableTags = [
    'Low GI',
    'Low Sugar',
    'High Fiber',
    'High Protein',
    'Keto-Friendly',
    'Low Sodium',
    'Under 30 Min'
  ];

  return (
    <div className="space-y-6 max-w-2xl pb-16">
      {/* Basics Card */}
      <div className="bg-white p-5 md:p-6 rounded-xl border border-outline-variant/40 shadow-[0px_4px_20px_rgba(45,49,48,0.05)] space-y-4">
        <h3 className="text-xs font-bold text-on-surface-variant uppercase tracking-widest border-b border-outline-variant/20 pb-2">
          Basics
        </h3>
        <div className="space-y-3">
          <Input
            id="recipe-title"
            label="Recipe Title"
            value={formData.title}
            onChange={(e) => handleBasicsChange('title', e.target.value)}
            placeholder="e.g. Herb-Roasted Chicken Bowl"
          />
          <div>
            <label htmlFor="recipe-description" className="block text-xs font-semibold text-on-surface-variant uppercase tracking-wider mb-1">
              Description
            </label>
            <textarea
              id="recipe-description"
              value={formData.description}
              onChange={(e) => handleBasicsChange('description', e.target.value)}
              className="w-full bg-white border border-outline-variant focus:ring-2 focus:ring-primary/20 focus:border-primary rounded-lg p-3 outline-none transition-all font-body-md text-on-surface text-sm"
              rows="3"
              placeholder="Describe the flavors, aromas, and blood-sugar balancing qualities..."
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <Input
              id="cooking-time"
              label="Prep + Cook Time (min)"
              type="number"
              value={formData.cookingTime}
              onChange={(e) => handleBasicsChange('cookingTime', parseInt(e.target.value) || 0)}
              min="1"
            />
            <Input
              id="servings"
              label="Yield (Servings)"
              type="number"
              value={formData.servings}
              onChange={(e) => handleBasicsChange('servings', parseInt(e.target.value) || 1)}
              min="1"
            />
          </div>
          <ImageUploader
            currentUrl={formData.imageUrl || ''}
            onUpload={(url) => handleBasicsChange('imageUrl', url)}
            onUrlChange={(url) => handleBasicsChange('imageUrl', url)}
          />
        </div>
      </div>

      {/* Category Tags Card */}
      <div className="bg-white p-5 md:p-6 rounded-xl border border-outline-variant/40 shadow-[0px_4px_20px_rgba(45,49,48,0.05)] space-y-4">
        <h3 className="text-xs font-bold text-on-surface-variant uppercase tracking-widest border-b border-outline-variant/20 pb-2">
          Dietary Tags
        </h3>
        <div className="flex flex-wrap gap-2">
          {availableTags.map(tag => {
            const isSelected = formData.tags.includes(tag);
            return (
              <TagChip
                key={tag}
                label={tag}
                active={isSelected}
                onClick={() => handleTagToggle(tag)}
              />
            );
          })}
        </div>
      </div>

      {/* Ingredient Selector Section */}
      <IngredientSelector
        ingredients={formData.ingredients}
        onChange={handleIngredientsChange}
        onAdd={handleAddIngredientPlaceholder}
        onRemove={handleRemoveIngredient}
      />

      {/* Steps Card */}
      <div className="bg-white p-5 md:p-6 rounded-xl border border-outline-variant/40 shadow-[0px_4px_20px_rgba(45,49,48,0.05)] space-y-4">
        <h3 className="text-xs font-bold text-on-surface-variant uppercase tracking-widest border-b border-outline-variant/20 pb-2">
          Instructions
        </h3>
        
        <div className="space-y-4">
          {formData.steps.map((step, idx) => (
            <div key={idx} className="flex gap-3 items-start relative group">
              <div className="w-8 h-8 rounded-lg bg-surface-container-highest flex items-center justify-center font-bold text-on-surface-variant text-xs shrink-0 mt-1">
                {String(idx + 1).padStart(2, '0')}
              </div>
              <div className="flex-1 bg-surface-container-low/30 p-3 rounded-lg border border-outline-variant/20 space-y-2 relative pr-8">
                <button
                  type="button"
                  onClick={() => handleRemoveStep(idx)}
                  aria-label={`Remove step ${idx + 1}`}
                  className="absolute top-2 right-2 text-on-surface-variant/60 hover:text-error transition-colors cursor-pointer"
                >
                  <span className="material-symbols-outlined text-[18px]">delete</span>
                </button>
                
                <input
                  type="text"
                  placeholder="Step Heading (e.g. Prepare Sauce)"
                  aria-label={`Step ${idx + 1} Heading`}
                  value={step.title}
                  onChange={(e) => handleStepChange(idx, 'title', e.target.value)}
                  className="w-full bg-transparent border-b border-outline-variant/30 focus:border-primary font-bold text-sm outline-none pb-1"
                />
                
                <textarea
                  placeholder="Describe step preparation tasks..."
                  aria-label={`Step ${idx + 1} Description`}
                  value={step.description}
                  onChange={(e) => handleStepChange(idx, 'description', e.target.value)}
                  className="w-full bg-transparent focus:ring-0 text-sm outline-none resize-none font-body-md"
                  rows="2"
                />

                <div className="flex items-center gap-1.5 pt-1">
                  <span className="material-symbols-outlined text-sm text-on-surface-variant">timer</span>
                  <input
                    type="number"
                    placeholder="Timer (min, optional)"
                    aria-label={`Step ${idx + 1} Timer in minutes`}
                    value={step.timer || ''}
                    onChange={(e) => handleStepChange(idx, 'timer', e.target.value)}
                    className="w-36 bg-white border border-outline-variant/40 rounded-lg px-2 h-7 text-xs outline-none"
                    min="1"
                  />
                </div>
              </div>
            </div>
          ))}
        </div>

        <button
          type="button"
          onClick={handleAddStep}
          className="w-full bg-surface-container-high/60 py-2.5 rounded font-label-md text-on-surface-variant uppercase text-[11px] font-bold hover:bg-surface-container-high transition-colors cursor-pointer border border-dashed border-outline-variant/50"
        >
          + Add Step
        </button>
      </div>

      {/* Form Submission Actions */}
      <div className="pt-4 border-t border-outline-variant/30 flex gap-4">
        <Button
          onClick={onPublish}
          className="flex-1 font-bold shadow-recipe"
        >
          Publish Recipe
        </Button>
        <Button
          onClick={onSaveDraft}
          variant="secondary"
          className="flex-1 font-bold"
        >
          Save as Draft
        </Button>
      </div>
    </div>
  );
};

export default EditorFormFields;
