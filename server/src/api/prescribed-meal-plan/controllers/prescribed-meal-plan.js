...
module.exports = createCoreController("api::prescribed-meal-plan.prescribed-meal-plan", ({ strapi }) => ({
  async find(ctx) {
    ...
    const { results, pagination } = await strapi.service("api::prescribed-meal-plan.prescribed-meal-plan").find(query);
  ...
  async findOne(ctx) {
    ...
    const entity = await strapi.service("api::prescribed-meal-plan.prescribed-meal-plan").findOne(ctx.params.id, query);
