...
| `fix/login-network-error` (PR #28) | `786c527d9a1ae28086ccfcf8fb1c7ba33ed6f8ab` | [PR #28](https://github.com/ux-fotisp/GlycoGourmet/pull/28) | **MERGED TO MASTER** (commit `786c527`): Full chain verified end-to-end; real browser login succeeded on Netlify Deploy Preview #28 with zero errors (`EVD-2026-010`). Merged by `ux-fotisp`. | 2026-09-06T07:16:39Z |
...
| **Open Login-Fix PR** | **OPEN (PR #28)** | PR #28 opened on branch `fix/login-network-error` with verified edge proxy and CSP configuration. | Complete. |
| **Merge Login-Fix PR** | **MERGED TO MASTER (commit 786c527)** | PR #28 merged to `master` at commit `786c527d9a1ae28086ccfcf8fb1c7ba33ed6f8ab`. Complete chain verified live: Netlify edge -> Render backend -> PostgreSQL -> Strapi auth -> JWT -> AuthContext -> onboarding screen (`EVD-2026-010`). *Sole external dependency:* Netlify production publishing paused due to billing credit exhaustion for the cycle (no overage billing risk; resumes automatically on cycle reset or plan upgrade). | Completed by Fotis P on 2026-09-06. |
...
| **Declare Login Resolved** | **RESOLVED / MERGED** | Real incognito browser login verified on Deploy Preview #28 with `demo-patient@glycogourmet.demo`; returns HTTP 200 + valid JWT and renders post-login onboarding screen without console errors. Status in `04-refine.md` promoted to PROMOTE; PR #28 merged to `master`. | Complete. |
