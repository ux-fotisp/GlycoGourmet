...
> [!NOTE]
> `clinic_admin` is strictly non-clinical. In accordance with `server/src/policies/is-clinic-admin.js` and `FORBIDDEN_CLINICAL_UIDS`, `canManageClients` evaluates to `false` to protect patient clinical telemetry from administrative access.
