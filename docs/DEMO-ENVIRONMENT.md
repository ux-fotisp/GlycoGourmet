...
3. **Container Idle Spin-Down & Client Cold-Start Resilience:**  
   The Render demo backend runs on Render's free tier. Under this infrastructure tier:
   - **Spin-Down Behavior:** The web service automatically spins down into an idle sleep state after **15 minutes** of HTTP inactivity. The first subsequent incoming request incurs a **30–60 second cold-start spin-up delay** while the container re-initializes.
   - **Client-Side Retry & Backoff (`strapiClient.js`):** To eliminate user-facing 502/504 gateway failures during this spin-up window, all frontend network requests routed through `strapiClient.js` are wrapped in an automated retry handler (`fetchWithRetry`). It executes up to **3 attempts with exponential backoff (2s → 5s → 10s)** strictly on network errors, timeouts, and HTTP 502/503/504 responses (client 4xx errors are never retried). During retries, a non-blocking toast indicator (`aria-live="polite"`, managed via `useBackendWakeStatus`) informs the user that the demo backend is waking up.
   - **Operational Keep-Alive Recommendation (External):** As a complementary operational measure (external to codebase code changes), administrators may configure an external uptime monitor or scheduled cron (e.g. via `cron-job.org`, BetterStack, or a scheduled GitHub Action) to ping `GET https://glycogourmet-demo-api.onrender.com/_health` every **10–14 minutes**. Because `/_health` returns `204 No Content` with zero database overhead, this keeps the container warm during active demo windows and avoids cold starts altogether.
